...
parser.add_argument ('--quick', '-q', dest='quick', action="store_true", default =False,
                     help='Stop after '+str(quickrun)+' rows. (Not yet implemented, and you don\'t want this anyway)')
...
def getITAdata(T1, T2, device, logger, testrange=-1, debug=False):
    ...
    for entry in entrySort:
        ...
        if i_entry >= len(entrySort): break

        if debug: print(entry)
        if not len(entry) == 3: continue
        ...
        time = pd.to_datetime(ts_string, dayfirst=True)
...
def main():
    ...
    total, df  = getITAdata(cmdT1, cmdT2, 'E:UTR112', 'MTA', debug)
